			...
			<Container>
				...
				<ol className="mt-12 grid gap-px border border-[var(--border)] bg-[var(--border)] sm:grid-cols-2 lg:grid-cols-5">
					{portfolioData.currentlyBuilding.map((item, index) => (
						<li
							key={item}
							className="building-item group flex min-h-40 flex-col justify-between bg-[var(--background)] p-5 transition-[background-color,border-color,transform] hover:bg-[var(--panel)] sm:p-6"
						>
							<span className="building-item__status flex items-center gap-2 font-mono text-xs text-[var(--accent)]">
